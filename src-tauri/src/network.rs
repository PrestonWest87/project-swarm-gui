        ...
        tokio::select! {
            ...
            Some(cmd) = rx.recv() => {
                match cmd {
                    ...
                    crate::NetworkCommand::JoinRoom(target_room) => {
                        let old_topic = gossipsub::IdentTopic::new(current_topic.clone());
                        let _ = swarm.behaviour_mut().gossipsub.unsubscribe(&old_topic);
            ...
            event = swarm.select_next_some() => match event {
                ...
                SwarmEvent::Behaviour(SwarmProtocolEvent::Gossipsub(gossipsub::Event::Message { message, .. })) => {
                    if let Ok(dag_msg) = serde_json::from_slice::<store::DagMessage>(&message.data) {
                        
                        // 1. Emit to the Svelte GUI first (while we still own the dag_msg reference)
                        ...
                        // 2. Move the dag_msg into the blocking SQLite thread
                        let db_clone = Arc::clone(&db);
